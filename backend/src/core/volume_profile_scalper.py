...
# Core imports - fixed path
from .models import Signal, OptionType, OrderSide, MarketRegime

# Simplified base class for compatibility
class BaseStrategy:
    """Simplified base strategy class"""
    def __init__(self, config=None):
        self.config = config or {}
        self.name = getattr(self, 'name', 'Strategy')
...
def calculate_value_area(prices, volumes):
    """Simplified value area calculation"""
    return {"poc": np.mean(prices), "va_high": np.max(prices), "va_low": np.min(prices)}
